########################
### 根据进化树自定义聚类结果（四类），对群体结构分析
########################
rm(list = ls())

library(tidyverse)

fam=read.table("snp.fam",sep = " ")
head(fam)

pop=read.table("./pop2.txt",header = T)
head(pop)
...
admix_res <- read.delim(paste("snp",4,"Q",sep = "."),sep = " ",header=F)
...
plot_k <- function(k){
  # k=3
  admix_res <- read.delim(paste("snp",k,"Q",sep = "."),sep = " ",header=F)
