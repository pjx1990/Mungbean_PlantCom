...
ggplot(mydata)+
  ...
  geom_rect(aes(xmin=xmin,xmax=xmax,
                ymin=-2.1,ymax=-0.1),
            colour="black", fill = "white")+
  geom_rect(data=BDC, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = 0, ymax=2, fill=Type)) +
  geom_rect(data=BRN, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -2.1, ymax=-0.1, fill=Type)) +
  ...
  # scale_x_discrete(position = "top", name = "Genomic Position", limits = mydata$chr)
  scale_x_continuous(position = "top", name = "", 
                 breaks=c(mydata$chr_label_pos),labels=c(mydata$chr))
...
col <- RColorBrewer::brewer.pal(7,"Set1")
###===================产量相关
# BRN
# PDL
# PDTN
...
ggplot(mydata)+
  ...
  geom_rect(data=PDL, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = 0, ymax=2, fill=Type)) +
  geom_rect(data=PDTN, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -2.1, ymax=-0.1, fill=Type)) +
  geom_rect(data=PDW, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -4.2, ymax=-2.2, fill=Type)) +
  geom_rect(data=SD100WT, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -6.3, ymax=-4.3, fill=Type)) +
  geom_rect(data=SDNPPD, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -8.4, ymax=-6.4, fill=Type)) +
  geom_rect(data=YPPL, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -10.5, ymax=-8.5, fill=Type)) +
  ...
  scale_y_continuous(breaks=c(1,-1.1,-3.2,-5.3,-7.4,-9.5),labels = yield_traits)
...
ggplot(mydata)+
  ...
  geom_rect(data=CPC, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = 0, ymax=2, fill=Type)) +
  geom_rect(data=CSC, aes(xmin = xmin,
                           xmax = xmax,
                           ymin = -2.1, ymax=-0.1, fill=Type)) +
  geom_rect(data=VITEXIN, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -4.2, ymax=-2.2, fill=Type)) +
  geom_rect(data=ISOVITEXIN, aes(xmin = xmin,
                              xmax = xmax,
                              ymin = -6.3, ymax=-4.3, fill=Type)) +
  ...
  scale_x_continuous(position = "top", name = "", 
                     breaks=c(mydata$chr_label_pos),labels=c(mydata$chr))+
  scale_fill_manual(values = col)+
  scale_y_continuous(breaks=c(1,-1.1,-3.2,-5.3),labels = quality_traits)
...
ggplot(mydata)+
  ...
  geom_rect(data=BR, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = 0, ymax=2, fill=Type)) +
  geom_rect(data=RFW, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -2.1, ymax=-0.1, fill=Type)) +
  ...
  scale_x_continuous(position = "top", name = "", 
                     breaks=c(mydata$chr_label_pos),labels=c(mydata$chr))+
  scale_fill_manual(values = col)+
  scale_y_continuous(breaks=c(1,-1.1),labels = resistance_traits)
...
ggplot(mydata)+
  ...
  geom_rect(data=BDC, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = 0, ymax=2, fill=Type)) +
  geom_rect(data=FLC, aes(xmin = xmin,
                           xmax = xmax,
                           ymin = -2.1, ymax=-0.1, fill=Type)) +
  geom_rect(data=PDC, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -4.2, ymax=-2.2, fill=Type)) +
  geom_rect(data=PLC, aes(xmin = xmin,
                              xmax = xmax,
                              ymin = -6.3, ymax=-4.3, fill=Type)) +
  geom_rect(data=SDC, aes(xmin = xmin,
                             xmax = xmax,
                             ymin = -8.4, ymax=-6.4, fill=Type)) +
  geom_rect(data=TLC, aes(xmin = xmin,
                           xmax = xmax,
                           ymin = -10.5, ymax=-8.5, fill=Type)) +
  geom_rect(data=YSC, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -12.6, ymax=-10.6, fill=Type)) +
  ...
  scale_x_continuous(position = "top", name = "", 
                     breaks=c(mydata$chr_label_pos),labels=c(mydata$chr))+
  scale_fill_manual(values = col)+
  scale_y_continuous(breaks=c(1,-1.1,-3.2,-5.3,-7.4,-9.5,-11.6),labels = color_traits)
...
ggplot(mydata)+
  ...
  geom_rect(data=PLH, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = 0, ymax=2, fill=Type)) +
  geom_rect(data=GRH, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -2.1, ymax=-0.1, fill=Type)) +
  geom_rect(data=HECL, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -4.2, ymax=-2.2, fill=Type)) +
  geom_rect(data=MLA, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -6.3, ymax=-4.3, fill=Type)) +
  geom_rect(data=MLL, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -8.4, ymax=-6.4, fill=Type)) +
  geom_rect(data=MLW, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -10.5, ymax=-8.5, fill=Type)) +
  geom_rect(data=MSD, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -12.6, ymax=-10.6, fill=Type)) +
  geom_rect(data=MSNN, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -14.7, ymax=-12.7, fill=Type)) +
  geom_rect(data=PDS, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -16.8, ymax=-14.8, fill=Type)) +
  ...
  scale_x_continuous(position = "top", name = "", 
                     breaks=c(mydata$chr_label_pos),labels=c(mydata$chr))+
  scale_fill_manual(values = col)+
  scale_y_continuous(breaks=c(1,-1.1,-3.2,-5.3,-7.4,-9.5,-11.6,-13.7,-15.8),labels = plant_architecture_traits)
...
ggplot(mydata)+
  ...
  geom_rect(data=FLD, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = 0, ymax=2, fill=Type)) +
  geom_rect(data=SDCG, aes(xmin = xmin,
                          xmax = xmax,
                          ymin = -2.1, ymax=-0.1, fill=Type)) +
  geom_rect(data=CHP, aes(xmin = xmin,
                              xmax = xmax,
                              ymin = -4.2, ymax=-2.2, fill=Type)) +
  ...
  scale_x_continuous(position = "top", name = "", 
                     breaks=c(mydata$chr_label_pos),labels=c(mydata$chr))+
  scale_fill_manual(values = col)+
  scale_y_continuous(breaks=c(1,-1.1,-3.2),labels = other_traits)
