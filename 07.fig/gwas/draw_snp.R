###########################
### GWAS gene PAV 全局概览图（全部性状整合）
###########################
rm(list = ls())
library(tidyverse)
library(scales)

bin <- read.delim("pav_geneGwas_fun.txt",header = T) %>% 
  filter(chr %in% seq(1,11))
head(bin)
...
colnames(bin_test)[1:5] <- c("Traits","Type","Chr","Rs","Pos")
